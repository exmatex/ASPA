...
#define TBOX_ERROR(X) do {					\
      ...
      tboxos << X << ends;					\
      MPTCOUPLER::toolbox::Utilities::abort(tboxos.str().c_str(), __FILE__, __LINE__);\
} while (0)
#else
#define TBOX_ERROR(X) do {					\
      ostrstream tboxos;					\
      tboxos << X << ends;					\
      MPTCOUPLER::toolbox::Utilities::abort(tboxos.str(), __FILE__, __LINE__);\
} while (0)
#endif
...
#define TBOX_WARNING(X) do {					\
      ostringstream tboxos;					\
      tboxos << X << ends;					\
      MPTCOUPLER::toolbox::Utilities::printWarning(tboxos.str(), __FILE__, __LINE__);\
} while (0)
#else
#define TBOX_WARNING(X) do {					\
      ostrstream tboxos;					\
